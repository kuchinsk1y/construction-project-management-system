...
function ComboBox({
  ...
  return (
    <div className={`relative ${className}`}>
      <div
        className={`flex items-center gap-1 w-full bg-[var(--background)] border rounded-md px-2 py-1 text-[11px] outline-none transition ${
          disabled
              ...
              {isEditMode ? (
                <>
                  ...
                  <Button
                    ...
                    className={`text-[11px] h-8 px-4 rounded-xl flex items-center gap-1.5 font-bold shadow-sm transition ${
                      canClose
            ...
            <table className="w-full border-collapse text-left text-xs">
              <thead className="sticky top-0 z-20 border-b border-[var(--border)] bg-[var(--background)]/95 backdrop-blur-md text-[10px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">
                <tr>
                  ...
                  <th className="px-2 py-2 border-r border-[var(--border)] min-w-[160px]">Rodz. rb.</th>
                ...
                {workTypesLoading ? (
                  ...
                  milestoneGroups.map(group => {
                    ...
                    const visibleRows = isEditMode
                      ...
                      : rows
                    const rowSpan = Math.max(1, visibleRows.length) + 1 // +1 for progress bar row
...
function MilestoneRows({
  ...
  if (rows.length === 0) {
    ...
    return (
      <>
        <tr className="bg-[var(--muted)]/10 align-middle">
        ...
        {isEditMode && canEdit && (
          <tr className="bg-[var(--sidebar-primary)]/3">
  ...
  return (
    <>
      {rows.map((row, idx) => {
        ...
        if ('wt' in row) {
          ...
          return (
            <tr
              key={wt.id}
              className="group hover:bg-[var(--muted)]/20 transition-colors align-middle"
            >
          ...
          return (
            <tr
              ...
              className={`align-middle transition-colors ${
                isRowInvalid ? 'bg-amber-500/5' : 'bg-[var(--sidebar-primary)]/5'
              }`}
            >
              {isFirst && milestoneCell(rowSpan)}
              {isFirst && descriptionCell(rowSpan)}
              ...
              <td className="px-1.5 py-1 border-r border-[var(--border)]">
                <ComboBox
                  value={editRow.name}
                  onChange={v => updateEditRow(editRow.id, 'name', v)}
              ...
              <td className="px-1.5 py-1 border-r border-[var(--border)]">
                <input
                  ...
                  max={100}
                  step={1}
                  className={`w-full text-center bg-[var(--background)] border rounded-md px-1 py-1 text-[11px] outline-none tabular-nums font-bold transition ${
                    isRowInvalid
      ...
      {/* Progress / add-row bar — always last row for this milestone group */}
      <tr className={`${isEditMode ? 'bg-[var(--sidebar-primary)]/3' : 'bg-[var(--muted)]/5'} border-t border-[var(--border)]/40`}>
