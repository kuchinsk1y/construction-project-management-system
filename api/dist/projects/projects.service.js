...
let ProjectsService = class ProjectsService {
    ...
    async createMilestone(projectId, dto) {
        ...
        const spreadsheetId = this.config.getOrThrow('PROJECTS_SPREADSHEET_ID');
        const sheetName = this.config.get('MILESTONES_SHEET_NAME', 'KamienieMilowe');
        const syncData = {
            id: row.id,
            projectId: project.id,
    ...
    async listForemen() {
        ...
        return users
            .filter((u) => u.roles.some((r) => r.toLowerCase() === 'foreman' || r.toLowerCase() === 'st. brygadzista'))
            .map((u) => ({
            id: u.id,
    ...
    async createWorkType(projectId, dto) {
        ...
        const spreadsheetId = this.config.getOrThrow('PROJECTS_SPREADSHEET_ID');
        const sheetName = this.config.get('WORK_TYPES_SHEET_NAME', 'RodzajeRobot');
        const syncData = {
            id: row.id,
            projectId: row.project_id,
