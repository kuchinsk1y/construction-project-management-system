...
export class UsersService {
  ...
  async remove(id: number) {
    ...
    if (existing.projects_projects_manager_idTousers.length > 0) {
      throw new BadRequestException(
        'Nie można usunąć użytkownika, ponieważ jest kierownikiem projektu',
      );
    }

    if (
      existing.projects_projects_created_byTousers.length > 0 ||
      existing.projects_projects_updated_byTousers.length > 0 ||
      existing.project_status_history.length > 0
    ) {
      throw new BadRequestException(
        'Nie można usunąć użytkownika, ponieważ posiada powiązane logi lub historię zmian projektów',
      );
    }
